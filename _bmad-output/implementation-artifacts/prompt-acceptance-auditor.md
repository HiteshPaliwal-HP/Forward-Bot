...
I want all recurring UI elements â€” degraded banners, log rows, filter icon rows, status pills, collapsible panels, and activation banners â€” implemented as reusable components with full accessibility support,
...
1. **DegradedBanner Component (UX-DR3):**
   ...
   - **And** this standalone `DegradedBanner` in `components/shared/` is distinct from the existing `components/layout/DegradedBanner.tsx` which is a self-contained, data-fetching banner â€” the shared version is a pure presentational component accepting props.
...
2. **LogRow Component (UX-DR4):**
   ...
   - **And** eight variants are implemented based on event name matching:
     - `forwarded` â†’ success stripe + `ArrowUpRight` icon (green)
     - `filter_blocked` â†’ muted stripe + `CircleSlash` icon (muted)
     - `telegram_rejected` â†’ error stripe + `AlertTriangle` icon (red)
     - `destination_unreachable` â†’ error stripe + `Ban` icon (red)
     - `flood_wait` â†’ warning stripe + `Clock` icon (amber)
     - `edit_propagated` â†’ success stripe + `Pencil` icon (green)
     - `delete_propagated` â†’ success stripe + `Trash2` icon (green)
     - `reply_orphaned` â†’ warning stripe + `CornerDownRight` icon with strikethrough modifier (amber)
     - All other events â†’ default neutral stripe + `Info` icon
   - **And** `LogRow` supports an expandable inline detail panel: clicking the row toggles display of the full raw JSON payload + a `correlation_id` copy button; if `rule_id` is present, a "Jump to rule â†’" link appears (clicking this link must use `e.stopPropagation()` to prevent toggling the panel).

3. **FilterIconRow Component (UX-DR5):**
   - **Given** `FilterIconRow` is rendered with a rule's filter configuration object.
   - **When** filters are active.
   - **Then** four lucide icons render in order: `Clock` (time_window), `Shuffle` (sampling), `Image` (media_type_filter), `Key` (keyword filters â€” block_keywords or allow_keywords).
   ...
   - **And** each icon has a hover Tooltip describing the configured value (e.g., `"Monâ€“Fri 09:00â€“17:00 Europe/Warsaw"` for time_window, `"Every 3rd message"` for sampling) using a custom `Tooltip` component built in `web/src/components/shared/Tooltip.tsx`.
   - **And** each icon hit area is a minimum of `24Ã—24px`.
...
5. **CollapsiblePanel Component (UX-DR7):**
   ...
   - **And** the chevron icon animates (rotates 180Â°) on open/close.
...
6. **ActivationBanner Component (UX-DR8):**
   ...
   - **And** the `ActivationBanner` is also used on the Dashboard to show first-run guidance when `isFirstRun={true}` â€” when no forwarding rules exist, it renders with a "Create your first rule" CTA that navigates to `/forwards/new`; `isFirstRun` prop overrides the `isActive` variant rendering.
...
- [x] **3. LogRow Component** (AC: 2)
  ...
  - [x] Create `web/src/components/shared/LogRow.tsx` with variant â†’ icon/stripe mapping for all 8 event types + default
  - [x] Implement expandable detail panel: click toggles full JSON payload view + correlation_id copy button + "Jump to rule â†’" link if rule_id present
...
- [x] **4. FilterIconRow Component** (AC: 3)
  ...
  - [x] Wrap each filter icon in the custom `Tooltip` component with descriptive configuration hover text; ensure 24Ã—24px minimum hit area
...
- [x] **6. CollapsiblePanel Component** (AC: 5)
  ...
  - [x] Use a stable `id` (derived from `title` or passed as prop) for `aria-controls` â†’ `id` linkage
...
- [x] **9. Verify Build** (AC: 9)
  - [x] Run `npm run build` in `web/` â€” must complete with zero TypeScript errors

---

## Dev Notes

### Critical: Shared vs. Layout DegradedBanner â€” DO NOT CONFUSE
...
- `time_window` active: `"Monâ€“Fri 09:00â€“17:00 Europe/Warsaw"` (format days + time + timezone)
...
- `web/src/components/shared/Button.tsx` â€” custom Button
- `web/src/components/shared/Sheet.tsx` â€” mobile nav drawer
...
The project has TypeScript strict mode enabled (`web/tsconfig.json`). All component props must be typed explicitly â€” no `any`. The build verification (`npm run build`) is the final gating check.

### Existing Token Classes to Reuse

From the review findings of Story 6-1, Tailwind v4 utility classes in this project work as:
- `bg-success-bg` â†’ `background-color: var(--color-success-bg)`
- `text-success-foreground` â†’ `color: var(--color-success-foreground)`
- `border-success-border` â†’ `border-color: var(--color-success-border)`
- `bg-warning-bg`, `text-warning-foreground`, `border-warning-border`
- `bg-error-bg`, `text-error`, `border-error-border`
- `text-muted-foreground` â†’ Tailwind's built-in which maps to `--muted-foreground` (which is `--color-text-secondary`)
- `text-active` â†’ maps to `--color-active`
...
- **`DegradedBanner` in layout** already self-manages health polling â€” do NOT modify `components/layout/DegradedBanner.tsx`.
- **`useNavigate()`** from `react-router-dom` is available in any component rendered within `BrowserRouter`.
- **Sonner** (`import { toast } from "sonner"`) is the toast library â€” already installed.
...
- **No Redux/Zustand** â€” only `ThemeContext` and (future) `SseContext` are global contexts per architecture F3. Do not introduce any new global state.

### Files Created in Story 6-1

 the following files exist and should be imported/referenced by Story 6-2 components:
- `web/src/lib/queryKeys.ts` â€” centralized key factory
- `web/src/lib/queryClient.ts` â€” TanStack QueryClient instance
- `web/src/api/client.ts` â€” Axios instance (use `apiClient` named export)
- `web/src/contexts/ThemeContext.tsx` â€” `useTheme()` hook
- `web/src/hooks/useAuth.ts` â€” `useAuth()` hook for session checks
- `web/src/components/layout/DegradedBanner.tsx` â€” LEAVE UNTOUCHED
...
```
38a29fe  6-1 story done  (Epic 6 Story 6.1 complete â€” React SPA foundation, brand tokens, auth, layout)
...
web/src/
â”œâ”€â”€ types/
â”‚   â””â”€â”€ ui.ts                           â† NEW: LogEntry, FilterConfig, StatusPillStatus interfaces
â”œâ”€â”€ components/
â”‚   â””â”€â”€ shared/
â”‚       â”œâ”€â”€ index.ts                    â† NEW: barrel export for all shared components
â”‚       â”œâ”€â”€ Tooltip.tsx                 â† NEW: custom Tooltip component
â”‚       â”œâ”€â”€ DegradedBanner.tsx          â† NEW: presentational (props-driven), distinct from layout/DegradedBanner
â”‚       â”œâ”€â”€ LogRow.tsx                  â† NEW: log event row with 8 variants + expandable detail
â”‚       â”œâ”€â”€ FilterIconRow.tsx           â† NEW: 4-icon filter status row with custom Tooltips
â”‚       â”œâ”€â”€ StatusPill.tsx              â† NEW: active/inactive/error/stale pill with icon+label
â”‚       â”œâ”€â”€ CollapsiblePanel.tsx        â† NEW: accordion panel with animated chevron + ARIA + Grid height transition
â”‚       â””â”€â”€ ActivationBanner.tsx        â† NEW: rule-inactive warning + first-run CTA variant
```
